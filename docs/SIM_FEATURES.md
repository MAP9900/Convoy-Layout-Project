...
- dataset inspection:
  ...
  - v4 tactical datasets also add counts by spawn region plus inside-convoy, target-aspect, target-score, nearest-ship-clearance, lead-solution diagnostics, and aim-offset diagnostics for intentional near/miss labels
  - `convoy_sim.profile_outcome_audit` runs generated records through moving zig-zag convoy outcome QA and reports or attaches intended-target hits, other-ship hits, closest passes, and outcome-vs-intent agreement before VAE training; its outcome gate is also reused during v4 generation
