...
Matched VAE source candidate-pool comparison:

Generated by `notebooks/vae_source_comparison.ipynb` with matched settings:
...
|---|---:|---:|---:|---:|---:|---:|---:|---:|
| Curated v4 | `0.9008` | `0.9166` | `2.8112` | `1287.7 m` | `1264.7 m` | `3.030` | `1.836` | `0.180` |
| Random v1 | `0.9170` | `0.8734` | `2.5412` | `2201.1 m` | `2051.5 m` | `2.894` | `2.013` | `0.189` |
| Mixed 70/30 | `0.8970` | `0.9094` | `2.7816` | `1564.9 m` | `1315.7 m` | `3.041` | `1.896` | `0.210` |
...
Interpretation before Monte Carlo ranking:

- All three sources can produce `1,000` safe accepted hit-threat candidates from `5,000` decoded samples under matched settings.
- Curated v4 remains the clean default: strong hit rate, balanced spawn-region coverage, and the existing post-fix full-state evaluation already looks good.
- Random v1 produces higher accepted mean clearance and higher unique ships hit, but lower pre-filter hit rate and lower accepted mean hits. It may be more spatially conservative and more dispersed.
- Mixed 70/30 produces the highest accepted mean hit count and largest inside-envelope share, but it is not clearly safer or more coherent than curated v4.
- The final source decision should use matched Monte Carlo scoring, not candidate-pool summaries alone.

Matched VAE source Monte Carlo evaluation:

Generated by `notebooks/vae_source_comparison.ipynb` with:

- evaluated candidates per source: `100`
...
|---|---:|---:|---:|---:|---:|---:|---:|---|---:|
| Curated v4 | `2.9223` | `1.6927` | `3.6732` | `1.7346` | `0.9787` | `5.1983` | `2.4747` | `VAE000046` | `6.4267` |
| Random v1 | `2.6923` | `1.8080` | `3.8420` | `1.8571` | `0.9693` | `5.7451` | `2.8053` | `VAE000018` | `7.3717` |
| Mixed 70/30 | `2.9820` | `1.8177` | `3.9180` | `1.8675` | `0.9997` | `5.6694` | `2.7240` | `VAE000124` | `7.9817` |

Interpretation after matched Monte Carlo:

- Mixed 70/30 is strongest on pool-average expected loss, pool hit reliability, pool expected hits, and best single candidate.
- Random v1 is strongest on top-25 expected loss and top-25 unique ships hit, suggesting its accepted candidates may be more dispersed across ships even though its pool-average hit count is lower.
- Curated v4 remains the best realism anchor, but it is no longer the strongest adversarial source under matched scoring.
- Recommended POMDP source: use the mixed 70/30 VAE pool as the primary GenAI candidate source, and keep random v1 as a stress-test comparison for unique-ship/value-spread behavior.
- For final paper claims, rerun the comparison with a larger evaluation budget after the POMDP path is working.
...
7. `notebooks/attack_candidate_pool_eval.ipynb` for single-pool drilldowns after the source comparison
