...
| `experiments/run_pomdp_candidate_selector.py` | `python -m experiments.run_pomdp_candidate_selector --candidate-path ...` | Candidate-pool JSONL, convoy profile, noisy observation settings | Belief-ranked candidate CSV/JSON plus metrics and manifest | `convoy_sim.pomdp_candidate_selector`, `convoy_sim.realism`, candidate JSONL records | Short | POMDP bridge / belief-limited selector baseline |
