...
| `experiments/generate_attack_profile_scaffold.py` | `python -m experiments.generate_attack_profile_scaffold --count ...` | Starting id, profile count, seed, convoy profile, output format/path | JSON-first scaffold payloads (plus optional `_scaffolded_fan_profile(...)` Python export) with generation-time audit filtering | `convoy_sim.attack_profiles`, `convoy_sim.profile_audit`, bounded realistic preset catalog | Short | Attack-profile authoring |
...
- `experiments/generate_attack_profile_scaffold.py`: Generates new attack-profile scaffolds from bounded convoy-attack presets so additional profiles can be authored without hand-picking every metric. Outputs JSON by default, samples `u_boat_initial_speed_mps` from `1.0` to `2.0` m/s in `0.1` steps, rejects spawns that start within `250 m` of any convoy ship, rejects profiles that fail the geometry plausibility audit against the selected convoy profile, and emits `_scaffolded_fan_profile(...)` calls when `--format python` is used for curated-library authoring. (`canonical`)
