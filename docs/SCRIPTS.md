...
- `experiments/generate_attack_profile_scaffold.py`: Generates new attack-profile scaffolds from bounded convoy-attack presets so additional profiles can be authored without hand-picking every metric. Outputs JSON by default, samples `u_boat_initial_speed_mps` from `1.0` to `2.0` m/s in `0.1` steps, and rejects profiles that fail the geometry plausibility audit against the selected convoy profile. (`canonical`)
