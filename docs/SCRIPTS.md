...
| `experiments/generate_attack_profile_scaffold.py` | `python -m experiments.generate_attack_profile_scaffold --count ...` | Starting id, profile count, seed, output format/path | Paste-ready Python `AttackProfile(...)` blocks or JSON scaffold payloads | `convoy_sim.attack_profiles`, bounded realistic preset catalog | Short | Attack-profile authoring |
...
- `experiments/generate_attack_profile_scaffold.py`: Generates new attack-profile scaffolds from bounded convoy-attack presets so additional profiles can be authored without hand-picking every metric. Defaults to moving U-boat profiles with `u_boat_initial_speed_mps = 2.0`. (`canonical`)
