...
| `experiments/generate_attack_profile_scaffold.py` | `python -m experiments.generate_attack_profile_scaffold --count ...` | Mode, starting id, profile count, seed, convoy profile, output format/path | Curated scaffold payloads or dataset-mode JSONL records with generation-time feasibility/audit filtering | `convoy_sim.attack_profiles`, `convoy_sim.profile_audit`, bounded realistic preset catalog | Short | Attack-profile authoring + dataset generation |
| `experiments/audit_attack_profile_dataset.py` | `python -m experiments.audit_attack_profile_dataset --input ...` | Dataset JSONL path, output dir | Flattened CSV plus distribution summaries for synthetic attack-profile datasets | JSONL dataset records from `generate_attack_profile_scaffold --mode dataset` | Short | Dataset audit / notebook prep |
| `experiments/train_vae.py` | `python -m experiments.train_vae ...` | Train/valid JSONL corpora plus VAE hyperparameters | VAE run artifacts under `results/runs/vae/` including metrics, manifest, history CSV, checkpoints, and decoded sample profiles | `convoy_sim.vae`, `torch`, workflow artifact helpers | Medium | VAE training |
...
- `experiments/generate_attack_profile_scaffold.py`: Shared attack-profile generator with two modes. `curated` preserves the current helper-style library workflow and can emit `_scaffolded_fan_profile(...)` calls for `attack_profiles.py`. `dataset` emits JSONL records for larger synthetic corpora and currently targets a `75% credible_hit_threat / 25% credible_near_miss` mix. Both modes sample `u_boat_initial_speed_mps` from `1.0` to `2.0` m/s in `0.1` steps, reject spawns that start within `250 m` of any convoy ship, and reject profiles that fail the geometry plausibility audit against the selected convoy profile. (`canonical`)
- `experiments/audit_attack_profile_dataset.py`: Audits JSONL synthetic attack-profile corpora, writes a flattened `profiles_flat.csv`, per-dimension count CSVs, and a summary JSON. The module functions can also be imported directly into a notebook for ad hoc inspection. (`diagnostic`)
- `experiments/train_vae.py`: First-pass beta-VAE training entrypoint for synthetic attack-profile corpora. Writes canonical run artifacts under `results/runs/vae/`, including `config_resolved.yaml`, `training_history.csv`, `metrics_summary.json`, `run_manifest.json`, `checkpoints/model_best.pt`, `checkpoints/model_latest.pt`, and `sampled_profiles.json`. Requires the ML environment with `torch` installed. (`canonical`)
